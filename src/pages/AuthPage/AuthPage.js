...
const LOG_IN_FIELDS = [
	{
		...
		rules: {
			...
			pattern: {
				value: EMAIL_PATTERN,
				message: `Use email pattern, like "test@gmail.com"`
			}
		}
	},
	{
...
export const AuthPage = () => {
	...
	const {mutate} = useMutation({
		...
		onSuccess: response => {
			const { idToken, localId, email } = response;
			// Handle errors
			if (!idToken && !localId) return alert(`Error ${response.error.message.toLowerCase()}`);

			dispatch(isLogInMode ? logIn(idToken, localId, email) : signUp(idToken, localId, email))
		},
